<div class="flex flex-col gap-4" x-data>
    ...
    <div class="px-4 py-4 shadow xl:px-72 lg:px-28">
        <h1 class="mb-5 text-2xl text-center">Gestion des volontaires de cepromor & aeph</h1>
        <div class="flex gap-4">
            <div class="flex flex-col flex-1 gap-4">
                <input type="text" placeholder="Noms du volontaire" class="p-2 border rounded" wire:model="noms">
                <input type="text" placeholder="Email" class="p-2 border rounded" wire:model="email">
                <textarea name="" id="" class="flex-1 p-2 border rounded" placeholder="La adresse du volontaire" wire:model="adresse"></textarea>
                <input type="text" placeholder="Numéro de conctact du volontaire" class="p-2 border rounded" wire:model="tel">
            </div>
    ...
    <div class="flex-1">
        <div class="grid grid-cols-4 gap-4">
            ...
            @endif
            @foreach ($volontaires as $tem)
                <a class="flex flex-col justify-center p-2 text-center shadow" href="#" wire:click="charger({{$tem}})">
                    @if (Storage::exists('public/volontaire/'.$tem->id.'.png'))    
                        <img src="{{asset('storage/volontaire/'.$tem->id.'.png')}}?{{ rand() }}" alt="Pas image pour cette volontaire" srcset="" class="flex-1">
                    @endif

                    <h1 class="text-lg font-bold ">{{$tem->noms}} / {{$tem->email}}</h1>
                    <p class="text-gray-400">{{$tem->adresse}}</p>
                    <p class="text-gray-400">{{$tem->tel}}</p>
                </a>
            @endforeach
        </div>
    </div>
